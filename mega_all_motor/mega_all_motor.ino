...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (command == 'M') { // Command to move joints
      ...
      float angle4 = Serial.parseFloat();
      float angle5 = Serial.parseFloat();
      float linearDistance = Serial.parseFloat(); // Read linear distance for Joint 6

      long steps1 = angleToSteps(angle1, GEAR_RATIO_XY); // No gear ratio for Joint 1
      long steps2 = angleToSteps(angle2, GEAR_RATIO_XY); // No gear ratio for Joint 2
      long steps3 = angleToSteps(angle3, GEAR_RATIO_JOINT_3);
      long steps4 = angleToSteps(angle4, GEAR_RATIO_JOINT_4);
      long steps5 = angleToSteps(angle5, GEAR_RATIO_JOINT_5);
      long steps6 = linearDistanceToSteps(linearDistance, LEAD_SCREW_PITCH); // For linear joint
      ...
      current_linearDistance = linearDistance;
      Serial.print("Moving joints to: ");
